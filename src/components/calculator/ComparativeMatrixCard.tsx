    ...
    <div className="bg-[#18181b] border border-[#27272a] rounded-2xl overflow-hidden shadow-sm">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs text-left rtl:text-right">
          ...
          <tbody className="divide-y divide-[#27272a]/60 font-mono-nums">
            {comparisonRows.map((row, idx) => (
              <tr key={idx} className="hover:bg-[#202024] transition-colors">
                <td className="py-3 px-4 font-sans font-medium text-[#f4f4f5]">
                  {row.metric}
                </td>
                <td className="py-3 px-4 text-[#a1a1aa]">{row.raw}</td>
                <td className="py-3 px-4 font-semibold text-[#f4f4f5]">
                  {row.adjusted}
                </td>
                <td
                  className={`py-3 px-4 text-right rtl:text-left font-semibold ${
                    row.isBad ? 'text-rose-400' : 'text-emerald-400'
                  }`}
                >
                  {row.variance}
                </td>
